...
def main(args):
    ...
    optim = optax.adam(schedule)
    non_lin = stax.elementwise(swish)
    n_hidden = [N_PARAM] * 4
    n_flow = 2
    print(f"Running ATESS w/ {n_flow} concatenated flows each w/ hidden layers={n_hidden} and Swish nonlinearity...")
    tic1 = pd.Timestamp.now()
